...
;; 5 PTS
(define (find-top_assist G x)
  (if (equal? G '()) (topsort-error)
      (if (equal? (indegree (get-edges x) (car G)) 0) (car G)
          (find-top_assist (cdr G) x))))
(define (find-top G)
  (find-top_assist (car G) G)
)
...
(define (pop-vertex G v)
  (make-graph (popv-vertices (get-vertices G) v)
              (popv-edges (get-edges G) v))
)
