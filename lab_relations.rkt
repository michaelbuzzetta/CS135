...
;; 5 PTS
(define (compose_help S R)
  (if (null? R) '()
      (union (map (lambda (A)(list(caar R) A)) (relate (cadar R) S)) (compose_help S (cdr R))))
  )
(define (compose S R)
   (compose_help S R)  
)
...
;; 5 PTS
(define (transitive_closure_help R A S)
  (if (eq? A 0) '()
      (union R (transitive_closure_help (compose S R) (- A 1) S) )
   )
)
(define (transitive-closure R)
   (transitive_closure_help R (cardinality R) R)
)
...
(define (involutive? n R)
   (if (set-equal?(compose R R) (id n)) #t
       #f)
