...
;; 2 PTS
(define (xor p q)
  (nand (not (and p (not(and p q)))) (not (and q (not (and p q)))))
)
...
(define (isosceles p q r)
  (or (and (and p q) (not r)) (or (and (and p r) (not q)) (and (and q r) (not p))))
)
...
;; 2 PTS
(define (yoda words)
  (cons (car (cddr words)) (cons (car words) (cons (cadr words) '())))
