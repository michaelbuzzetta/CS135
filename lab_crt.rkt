...
(define (CTR-exists_assist L i)
  (if (null? L)#t
      (if (equal? 1 (gcd i (car (cdr (car L)))))
      (CTR-exists_assist (cdr L) i) #f))
  )

(define (CRT-exists? L)
  (if (null? L)#t
      (if (CTR-exists_assist (cdr L) (car (cdr (car L))))
          (CRT-exists? (cdr L))#f
         )))
...
(define (CRT-helper L m)
  (if (null? L) 0
      (+ (CRT-helper (cdr L) m) (* (car (car L)) (/ m (car (cdr (car L))))
                                   (mod-inverse (/ m (car (cdr (car L))))
                                                (car (cdr (car L))))))
))
...
(define (CRT L)
  (if (CRT-exists? L)
      (modulo (CRT-helper L (find-m L)) (find-m L)) -1)
)
