...
(define (waterjug_assist fA fB cA cB g)
(if (equal? fB g) (list(list fA fB))
    (if (equal? fB cB) (append (list(list fA fB)) (waterjug_assist fA 0 cA cB g))
        (if (equal? fA 0) (append (list(list fA fB)) (waterjug_assist cA fB cA cB g))
            (if (> (+ fA fB) cB) (append (list(list fA fB)) (waterjug_assist (remainder (+ fA fB) cB) cB cA cB g))
                (if (<= (+ fA fB) cB) (append (list(list fA fB)) (waterjug_assist 0 (+ fA fB) cA cB g)) #f)))))
)

(define (waterjug cA cB g)
  (if (wj-solvable? cA cB g)
  (waterjug_assist 0 0 cA cB g) #f
))
...
;; 5 PTS
(define (factorize_assist n p)
  (if (>= p n) (list n)
  (if (divides? p n) (append (list p) (factorize_assist (/ n p) 2))
      (factorize_assist n (+ p 1)))
  ))

  
(define (factorize n)
  (factorize_assist n 2)
)
...
;; 3 PTS
(define (prime? n)
  (if (equal? (length (factorize n)) 1) #t
      #f) 
)
