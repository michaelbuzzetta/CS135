...
;; 5 PTS
(define (pulverizer_assist a x y b z w)
    (if (= a 0)
        (list z w)
        (let ((quotient (quotient b a)))
          (pulverizer_assist (- b (* quotient a))
                             (- z (* quotient x))
                             (- w (* quotient y))
                             a x y))))
(define (pulverizer_assist_2 a b)
  (pulverizer_assist a 1 0 b 0 1))

(define (pulverizer a b)
  (cond ((> b a) (pulverizer_assist_2 b a))
      (else(pulverizer_assist_2 a b))))
...
(define (error-inverse)
  (eopl:error "Error computing inverse: a and m must be relatively prime"))
...
(define (mod-inverse a m)
  (cond
    ((= 1 (gcd a m))(modulo (cadr (pulverizer a m)) m))
    (else (error-inverse))
  )
)
