...
(define (generate p q e)
  (append (list (list (* p q) e))
          (list (list (* p q) (mod-inverse e (lcm (- p 1) (- p 1))))))
)
...
(define (mod-exp b e m)
  (if (equal? e 0) 1
      (if (equal? (modulo e 2) 0)
          (modulo (expt (modulo (mod-exp b (/ e 2) m) m) 2) m)
...
;; 6 PTS
(define (encrypt_assist x y)
  (if (equal? x '()) '()
      (append (list (mod-exp (char->integer (car x)) (car (cdr y)) (car y)))
              (encrypt_assist (cdr x) y)))
  )

(define (encrypt plaintext key)
  (encrypt_assist (string->list plaintext) key)
)
...
(define (decrypt ciphertext key)
  (list->string(decrypt_assist ciphertext key))
  )

(define (decrypt_assist x y)
  (if (equal? x '()) '()
      (append (list(integer->char (mod-exp (car x) (car (cdr y)) (car y)))) (decrypt_assist (cdr x) y)))
  )
